@foreach($pages as $page)
    <li class="{{ Request::is($page->uri_wildcard) ? 'active' : '' }} {{ count($page->children) ? ($page->isChild() ? 'dropdown-submenu' : 'dropdown') : '' }}">
        <a href="{{ url($page->uri) }}" class="{{count($page->children) ? ($page->isChild() ? 'dropdown-toggle' : 'dropdown-toggle') : ''}}" data-toggle="{{count($page->children) ? ($page->isChild() ? 'dropdown' : 'dropdown') : ''}}">
            {{ $page->title }}
